...
import { getEditorPlugins } from '../../components/Editor/editorOptions'
...
export class PluginStore {
	...
	*initProjectPlugins(projectType: ProjectType) {
		...
		this.projectPlugins = [...localPlugins]
	}

	getPluginsToLoad(projectType: ProjectType): EditorProps['plugins'] {
		return [...getEditorPlugins(projectType), ...(this.projectPlugins || [])]
	}
...
export const loadInstalledPlugins = () =>
	new Promise<InstallablePlugin[]>(async res => {
...
export const installPlugin = (plugin: InstallablePlugin) =>
...
export const uninstallPlugin = (plugin: InstallablePlugin) =>
	new Promise<void>(async res => {
		try {
			const installed = await loadInstalledPlugins()
