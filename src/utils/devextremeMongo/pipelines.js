...
const createGroupKeyPipeline = (selector, groupInterval, groupIndex, contextOptions) => {
...
const createGroupStagePipeline = (includeDataItems, countingSeparately, itemProjection, groupKeyPipeline) => {
	let result = {
		$group: {
...
const createMatchPipeline = (selector, value) => [{ $match: { [selector]: value } }]
...
const isAndChainWithIncompleteAnds = element => {
	...
	return element.reduce(
		(r, v) => r && ((typeof v === 'string' && v.toLowerCase() === 'and') || Array.isArray(v)),
		true,
	)
}
...
const fixAndChainWithIncompleteAnds = element => Array.from(_fixAndChainWithIncompleteAnds(element))
	...
	if (typeof element === 'string') {
		...
		if (element.length === 1 && Array.isArray(element[0])) {
			...
			if (element[0] === '!') {
				...
				return parseFilter(fixAndChainWithIncompleteAnds(element), contextOptions)
			else return null
		} else {
			if (isAndChainWithIncompleteAnds(element))
				...
				if (['and', 'or'].includes(operator)) {
					...
					if (element.length === 3) {
						...
						switch (operator) {
							case '=':
								return rval(construct(fieldName, '$eq', element[2]), [element[0]])
							case '<>':
								return rval(construct(fieldName, '$ne', element[2]), [element[0]])
							case '>':
								return rval(construct(fieldName, '$gt', element[2]), [element[0]])
							case '>=':
								return rval(construct(fieldName, '$gte', element[2]), [element[0]])
							case '<':
								return rval(construct(fieldName, '$lt', element[2]), [element[0]])
							case '<=':
								return rval(construct(fieldName, '$lte', element[2]), [element[0]])
							case 'startswith':
								return rval(constructRegex(fieldName, '^' + element[2], caseInsensitiveRegex), [
									element[0],
								])
							case 'endswith':
								return rval(constructRegex(fieldName, element[2] + '$', caseInsensitiveRegex), [
							...
							case 'contains':
								return rval(constructRegex(fieldName, element[2], caseInsensitiveRegex), [element[0]])
							case 'notcontains':
								return rval(
							...
							case 'equalsobjectid':
								return rval(construct(fieldName, '$eq', new ObjectId(element[2])), [element[0]])
							default:
								return null
						}
					} else return null
...
const createSortPipeline = sort =>
	sort
		? [
				{
					$sort: sort.reduce((r, v) => ({ ...r, [v.selector]: v.desc ? -1 : 1 }), {}),
				},
			]
		: []
...
const createCompleteFilterPipeline = (searchExpr, searchOperation, searchValue, filter, contextOptions) => {
	// this pipeline has the search options, the function also returns
	// a list of fields that are being accessed
	const searchPipeline = createSearchPipeline(searchExpr, searchOperation, searchValue, contextOptions)
