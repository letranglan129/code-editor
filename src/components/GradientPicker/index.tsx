...
export default function GradientPicker({ className, value, onChange }: Props) {
			...
			<div className="relative mb-6 mt-3">
				...
				{stops.map((stop, i) => (
					<div key={i} className="absolute top-0 h-full" style={{ left: `${stop.hint}%` }}>
						<div
							className="cursor-pointer absolute translate-x-[-50%] top-[-14px]"
						...
						<div
							...
							className={cx(
								'h-full w-[4px] translate-x-[-50%] cursor-colr opacity-50 border-white',
								br.br,
								br.bl,
								isSelected(stop) ? 'bg-white' : 'bg-black',
							)}
						/>
						<div className="cursor-pointer bottom-[-17px] left-[-7px] absolute">
							<ColorPicker
								value={stop.color}
								onChange={(v, p) => updateColor(stop, v, p)}
