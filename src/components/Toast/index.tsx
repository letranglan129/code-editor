...
export const Toast = React.forwardRef<HTMLDivElement, ToastProps>(function Toast(props: ToastProps, ref) {
	const {
		children,
		className,
		content,
	...
	return (
		...
		>
			<Grid space="m" items="center" className={pad.y}>
				...
				<GridItem grow>
					<Grid col space="xs">
						<GridItem className="text-lg font-medium leading-6">{props.header}</GridItem>
						<GridItem className={cl.txtLowEmphasis}>{content || children}</GridItem>
...
export const ToastContainer = observer(
	React.forwardRef<HTMLDivElement, ToastContainerProps>(function ToastContainer(
		{ className, toasts, onDismiss, ...props }: ToastContainerProps,
		ref,
	) {
