...
export default function StackItem(props: any) {
	...
	return (
		<div ref={setNodeRef} style={style} {...attributes}>
			<Grid
				space="s"
				key={item.getIndex ? item.getIndex() : item.id || item.cid}
				onClick={onSelect}
				className={className}
				items="center"
				nowrap
			>
				...
				<GridItem grow>
					<Grid>
						<GridItem grow>{renderItem(item)}</GridItem>
						{!!previewStyle && (
							<GridItem>
								<div
									className={cx('w-[15px] h-[15px] bg-white', br.b, br.rnd)}
