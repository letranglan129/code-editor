		...
		>
			<Tab.List as={Grid} className={cx(classBgList)} full nowrap>
				{tabs.map(tab => (
					...
					<Tab
						className={({ selected }) =>
							cx('flex-1 focus:outline-none', classTab(selected), selected ? `${cl.tAo} ${cl.bA}` : cl.br)
						}
						key={tab.id}
					>
						{tab.label}
			...
			<Tab.Panels className={cx('hdl-tab-panels overflow-hidden flex-1 mt-2', classNamePanel)}>
				{tabs.map(tab => (
					<Tab.Panel key={tab.id} tabIndex={-1} className="hdl-tab-panel h-full" ref={clearTabIndex}>
						{tab.content}
					</Tab.Panel>
				))}
			</Tab.Panels>
