	...
	return (
		<Card padding={false} className={cx('overflow-hidden max-w-full group', className)}>
			<Grid key={rule.cid} col space="xs">
				<Grid
					className="bg-checker p-3 overflow-auto overflow-x-hidden h-[75px]"
					justify="center"
					items="center"
				>
						...
						{!!ruleDevice && ruleDevice.getWidthMedia() && (
							<GridItem>
								<Tooltip
									title={
										<>
											{i18nStore.t('deviceManager.device')}: <b>{ruleDevice.getName()}</b>
										</>
									}
								>
						...
						{!!ruleState && (
							<GridItem>
								<Tooltip
									title={
										<>
											{i18nStore.t('selectorManager.state')}: <b>{ruleState.getLabel()}</b>
										</>
									}
								>
		...
		<Grid col space="m">
			...
			<GridItem className="h-[75vh] w-full overflow-auto">
				{!filteredRules.length ? (
					<Grid justify="center">{i18nStore.t('modals.styleCatalog.noStyles')}</Grid>
				) : (
					<VirtuosoGrid
						key={tid}
						data={filteredRules}
