...
import wireframeProject from './templates/landing.json'
		...
		<Card padding={false} className={cx('overflow-hidden max-w-full group', className)}>
			<Grid col>
				...
				<Grid className="p-1 max-w-full" col space="s">
					<GridItem className={cx(fx.txtEllips, 'text-sm max-w-full')}>{project.name}</GridItem>
					<Grid items="center">
		...
		<Grid col full space="m">
			...
			<GridItem grow>
				{!projects ? (
					...
					<Grid justify="center">{t('notFound')}</Grid>
				) : (
					<VirtuosoGrid
						key={tid}
						data={projects}
