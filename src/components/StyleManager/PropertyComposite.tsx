...
const optsType = [{ id: 'all', label: <Icon path={mdiBorderAllVariant} size={icon.sx} />, title: 'All' }, customOption]
				...
				<PropertyLabel {...mainPropProps}>
					<SectorRow bottom="0">
						{children !== undefined ? (
							...
							<>
								<GridItem grow>{renderInput(prop1, { same: valueAllSame, all: allProps })}</GridItem>
								<GridItem grow>
									<ButtonGroupField
										size="s"
										options={optsType}
