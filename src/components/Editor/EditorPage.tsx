...
export default observer(function EditorPage() {
	...
	const { projectIdToLoad, editor, updateAppShell, analytics, isDev, editorKey, isAuthorized, setIsAuthorized } =
		appEditorStore
	...
	return (
		<Grid className={cx('relative transition-colors h-full', cl.bg, cl.txt)} justify="center" items="center" col>
			{isAuthorized ? <>{!!editorKey && <EditorApp />}</> : <div>{i18nStore.t('unauthorized')}</div>}
