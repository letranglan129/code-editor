...
import { ThemeOptions, useLocalSettingsStore } from '../../store/builder/localSettings'
	...
	const infoItems: MenuListItem[] = [
		...
		},
		{
			...
			action: async () => {
				...
				const fileName = `Untitled-${new Date()
					.toISOString()
					.replace('T', '-')
					.replace(/:|\./g, '')
					.substring(0, 17)}`
			...
			label: (
				<Grid items="center" space="m">
					...
					<GridItem>
						<Tabs
							tabs={themeTabs}
							onChange={value => setTheme(value as ThemeOptions)}
							value={theme}
							variant="pills"
						/>
					</GridItem>
		...
		<Grid space="s" items="center" justify="end" className={cx(pad.xS)}>
			...
			<GridItem>
				<Popover
					...
					title={false}
					padding={false}
					handler={<Icon path={mdiCog} size={icon.s} />}
					overlay
				>
					{({ close }) => (
						<>
