...
export default observer(function AssetManager({
	assets,
	select,
	close,
}: Omit<AssetsResultProps, 'Container' | 'open' | 'types'>) {
			...
			<Grid items="center" space="m">
				...
				<GridItem>
					<Popover
						...
						buttonAs="span"
						handler={
							<Button variant="pr" size="m2">
								{i18nStore.t('assetManager.addUrl')}
							</Button>
						}
					>
						{({ close }) => (
							<Grid space="m">
								<GridItem>
									<InputField
										...
										placeholder={i18nStore.t('assetManager.inputPlh')}
									/>
								</GridItem>
								<GridItem>
									<Button variant="pr" size="m2" onClick={handleUrlAdd(close)}>
										{i18nStore.t('add')}
									</Button>
								</GridItem>
							</Grid>
