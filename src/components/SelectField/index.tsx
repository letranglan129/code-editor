...
const getPosValues = (open: boolean, refBtnEl: HTMLElement | null, refPanelEl: HTMLElement | null) => {
	...
	const style =
		open && refBtnEl && refPanelEl ? calculatePosition(refBtnEl, refPanelEl, { position: { y: 'bottom' } }) : {}
	return {
		...style,
...
export const optionsClassName = cx('cursor-default select-none focus:outline-none', 'z-20 max-h-full')
...
export function SelectFieldItem(props: {
	option: Option
	active?: boolean
	selected?: Option
	isSelected?: () => boolean
	...
	return (
		<Grid items="center" className={cx([pad.xy, active && pop.bgH])}>
			...
			<GridItem className="pl-2">
				{isSelected(option, selected) && isSelectedAdd && (
					<Icon className={cl.tA} path={mdiCheck} size={icon.s} />
				)}
			</GridItem>
		</Grid>
	)
					...
					<div className="relative w-full text-sm">
						...
						>
							<Card
								padding={false}
								className="h-full"
								style={{ minWidth: `${refButton?.offsetWidth}px`, maxHeight: 200 }}
							>
								{!opts.length && search && search !== true && (
									<div className={pad.xy}>{search.empty}</div>
								)}
									...
									<Listbox.Option key={index} value={option}>
										{({ active, selected: sel }) => (
											<Grid items="center" className={cx([pad.xy, active && pop.bgH])}>
												...
												<GridItem className="pl-2">
													{isSelected(option, selected) && !hasSearch && (
														<Icon className={cl.tA} path={mdiCheck} size={icon.s} />
													)}
												</GridItem>
											</Grid>
