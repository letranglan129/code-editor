			...
			{hasSomeTarget ? (
				<>
					<Grid items="stretch" space="xs" className={cx(clsType, !isTargetComponents && clsTypeA)}>
						<Tooltip
							title={t(hasSelectors ? 'applyOnSelector' : 'noSelectors')}
									...
									<>
										...
										{selectors.map(s => (
											<ClassTag
												...
												toggleable={
													hasComponents &&
													selectors.filter(sel => sel.getActive() || sel === s).length > 1
												}
											/>
										))}
									</>
					...
					<Grid items="stretch" space="xs" className={cx(clsType, isTargetComponents && clsTypeA)}>
						<Tooltip
							title={t(hasComponents ? 'applyOnComponents' : 'noComponents')}
					...
					<Grid space="s" items="center" justify="between">
									...
									{!!targetDevice && targetDevice.getWidthMedia() && (
										<>
											<Tooltip
												title={
													<>
														{t('device')}: <b>{targetDevice.getName()}</b>
													</>
												}
											>
										...
										title={
											<>
												...
												<span className="text-xs opacity-75">
													{' '}
													{target?.selectorsToString({ skipState: 1 })}
												</span>
											</>
										}
									>
									...
									{!!targetState && (
										<>
											...
											<Tooltip
												title={
													<>
														{t('state')}: <b>{targetState.getLabel()}</b>
													</>
												}
											>
						...
						<ButtonWithTooltip
							title={i18nStore.t('toggleCss')}
							onClick={toggleCssPanel}
							active={isCssPanelActive}
						>
